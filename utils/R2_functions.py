...
def gvf_R2(xsample, eta, xtraj, xdottraj):
    ...
    distance = torch.zeros([xsample.shape[0],xtraj.shape[0]]).to(xtraj)
    
    # xsamples : (n, 2)
    # xtraj : (m, 2)
    
    xsample_re = xsample.unsqueeze(1).repeat(1, xtraj.shape[0], 1)
    xtraj_re = xtraj.repeat(xsample.shape[0], 1, 1)
    distance = torch.norm(xsample_re-xtraj_re, dim=2)
    
    index_closest = torch.argmin(distance, dim=1)
    closest_point = xtraj[index_closest] # dim = (1000)
    
    V1 = xdottraj[index_closest].to(xtraj)
    vel = closest_point.to(xtraj) - xsample.to(xtraj)
